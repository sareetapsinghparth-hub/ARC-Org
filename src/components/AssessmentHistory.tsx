...
export function AssessmentHistory({ history, concepts = [] }: AssessmentHistoryProps) {
  ...
  const chartData = useMemo(() => {
    ...
    history.forEach((item, index) => {
      ...
      // Calculate running overall average across all assessed concepts so far
      ...
      // Set current confidence for assessed concepts
      availableConcepts.forEach((c) => {
        if (runningConceptConfidence[c.id] !== undefined) {
          point[c.name] = runningConceptConfidence[c.id];
          point[`${c.id}_conf`] = runningConceptConfidence[c.id];
        }
      });
  ...
  if (history.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-6 text-center text-xs text-slate-500 space-y-2 shadow-xs">
        <ChartIcon className="w-8 h-8 mx-auto text-slate-400 opacity-60" />
        <p className="font-medium text-slate-700">Diagnostic Session Timeline & Trend Chart</p>
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-sm space-y-4">
      ...
      {activeTab === 'chart' && (
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-1.5 text-slate-500">
            ...
            <span className="font-semibold text-slate-700">Filter Concept:</span>
          </div>
      ...
      {activeTab === 'chart' && (
        <div className="space-y-3">
          <div className="h-64 sm:h-72 w-full bg-slate-50 border border-slate-200 rounded-xl p-3 sm:p-4 relative">
          ...
          {history
            ...
            .map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-xs space-y-2 hover:border-slate-300 transition-colors"
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 shrink-0 font-mono text-[11px]">
                      {item.imageAttached && (
                        <span className="flex items-center gap-1 text-indigo-700 bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-200 font-medium">
                  ...
                  <p className="text-slate-800 text-xs bg-white p-2 rounded-lg border border-slate-200 font-mono line-clamp-2">
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-xl rounded-xl p-3 text-xs max-w-xs space-y-2 z-50">
      ...
      {!isStart && dataPoint.delta !== undefined && (
        <div className="pt-1.5 border-t border-slate-100 text-[10px] flex items-center justify-between">
          <span className="text-slate-500">Step Delta:</span>
          <span
            className={`font-mono font-bold ${
              dataPoint.delta >= 0 ? 'text-emerald-700' : 'text-rose-700'
            }`}
          >
            {dataPoint.delta >= 0 ? '+' : ''}
